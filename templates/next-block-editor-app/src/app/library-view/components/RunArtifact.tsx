...
export function RunArtifact({
  ...
  const renderContent = () => {
    ...
    switch (type) {
          ...
          >
            {metadata?.thumbnail && (
              <div className="w-full h-32 bg-slate-600 rounded-t-md overflow-hidden">
                <img src={metadata.thumbnail} alt="Website preview" className="w-full h-full object-cover" />
              </div>
            )}
            <div className="p-3 bg-slate-600 rounded-b-md flex items-center gap-2">
              <ExternalLink className="w-4 h-4 text-slate-300" />
              <span className="text-sm text-slate-300 truncate">{metadata?.url || output}</span>
            </div>
          </a>
        )
      ...
      case 'code':
        ...
        return (
          <div className="space-y-2">
            ...
            <pre
              className={`text-xs font-mono bg-slate-800 p-3 rounded-md overflow-x-auto ${!isExpanded ? 'max-h-[96px] overflow-y-hidden' : ''}`}
            >
              {displayedOutput}
            </pre>
            {hasMoreLines && (
      ...
      case 'message':
        ...
        return (
          <div className="space-y-2">
            ...
            <div
              className={`text-sm text-slate-300 bg-slate-800 p-3 rounded-md whitespace-pre-wrap ${!isExpanded ? 'max-h-[96px] overflow-y-hidden' : ''}`}
            >
              {displayedMessage}
            </div>
            {hasMoreMessageLines && (
            ...
            {metadata?.summary && <div className="text-xs text-slate-400 mt-2">{metadata.summary}</div>}
          </div>
        )
        ...
        return (
          <div className="space-y-2">
            <div className="w-full rounded-md overflow-hidden bg-slate-600">
              <img src={output} alt="Generated media" className="w-full h-full object-cover" />
            </div>
            {metadata?.summary && <div className="text-xs text-slate-400 mt-2">{metadata.summary}</div>}
          </div>
        )
      ...
      default:
        return <div className="text-sm text-slate-300">{output || 'Output goes here'}</div>
    }
  }
